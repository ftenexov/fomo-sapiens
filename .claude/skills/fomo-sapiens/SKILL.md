...
3. **Relay in chat** (login.py prints all of it): the **account handle**, the **balance**, and the **ledger line** if one appears — once trading is enabled the skill registers the account on the agent ledger under its fomo handle, keyed by the wallet address and **proven by a signature (the fomo token is never sent)**; tell the user in one sentence that their trades will be tracked for PnL/leaderboard and that `ledger off` opts out at any time. Registration needs the signing key, so it happens at the first trade, not at login — there may be no ledger line yet, which is fine. **If the account is empty, show the deposit addresses** (Solana + EVM) and tell them to deposit ≥ their intended trade size (funds convert to Solana USDC).
