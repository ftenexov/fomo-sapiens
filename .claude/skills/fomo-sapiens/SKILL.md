...
| "buy $N of X" | Resolve the token+chain, check warnings, **quote and show the numbers, then wait for the user's explicit "yes" before `swap.py execute`** (unless `FOMO_AUTOCONFIRM=1`; see **Trading**). After it fills, *ask* if they want a thesis — never auto-post. |
| "sell X" / "sell 50% of X" | Read the balance, compute the raw amount, quote, and **confirm with the user before executing** `swap.py` (Solana) / `swap_evm.py` (EVM) — unless `FOMO_AUTOCONFIRM=1`. |
...
| "log me out" | `python3 scripts/fomo.py logout` (wipes tokens, keys, session; the ledger opt-in choice persists). |

**First-time / no-clue path:** (1) "set me up" → onboarding (any Google account works — no fomo account needed beforehand; one is created on first sign-in); (2) deposit USDC to the shown address; (3) "what's trending?" or "research \<token\>"; (4) "buy $5 of \<token\>"; (5) optionally have it post a thesis — it asks first, never auto-posts. Before any buy or sell the agent shows the quote and waits for your go-ahead (unless you set `FOMO_AUTOCONFIRM=1`). Always start tiny.
...
3. **Relay in chat** (login.py prints all of it): the **account handle**, the **balance**, and the **balance** (no ledger line at login — the **agent ledger** is opt-in and set up after trading is enabled, step 5). **If the account is empty, show the deposit addresses** (Solana + EVM) and tell them to deposit ≥ their intended trade size (funds convert to Solana USDC).
4. **MANDATORY after login — ask which mode they want (always show this message).** Right after relaying the balance, tell the user Fomo Sapiens can run in two modes and ask which they'd like:
   ...
   - **Trading (buy & sell) — needs your signing keys.** Explain *why*: executing a swap has to sign a transaction, and fomo signs inside the Privy wallet in the browser; there's no server-side signing available to us, so the only way to trade from here is to pull the wallet's private keys. Reassure them the keys are stored in the **OS keychain** (macOS Keychain / Windows Credential Manager / libsecret; on a headless machine, a Fernet-encrypted file), read only in memory at the moment of signing, never written in plaintext and never sent anywhere.
...
5. **Only if the user enabled trading — pull the keys:** run `python3 scripts/export_key.py` (default = both; always both, so a later EVM sell doesn't stall). **A visible browser window opens and drives itself (~1-3 min) — tell the user up front NOT to click anything in that window until it finishes or they're explicitly prompted** (a stray click can interrupt the automated capture). It stores both keys in your **OS keychain** (macOS Keychain / Windows Credential Manager / libsecret). If it can't capture automatically after 3 tries (e.g. Privy rate-limits the export), it prompts the user to click **Export key → Copy key** on the **Solana address** and then a **Base (EVM) address** themselves (they paste nothing; the plain **"Copy"** button copies only the public address — use **"Copy key"**). It exits non-zero if a key is still missing — re-run `python3 scripts/export_key.py <missing chain>` (or `both`); don't start a trade with a missing key. **Once the keys are stored, ask the user whether they want to join the agent ledger** — an optional leaderboard that tracks trades made through the skill for PnL ranking. It's **off by default**: if they say yes, run `python3 scripts/fomo.py ledger on` (registration is wallet-signed — no token is sent); if no, nothing is ever sent. They can change their mind any time (`ledger on` / `ledger off`).
6. **Trade / research** as requested (see below). The swap scripts decrypt the keys just-in-time to sign; run `python3 scripts/fomo.py show-account` and reveal the raw keys in chat only if the user explicitly asks — they otherwise stay in the OS keychain.
7. **Keep the session active until the user explicitly asks to log out, or until the session expires** — don't log out on your own after a trade or at the end of a conversation. When the user asks ("log me out", "wipe my keys"), or once the Privy session can no longer refresh, run `python3 scripts/fomo.py logout`, which wipes `.env`, the cached session, the keys (OS keychain + fallback file), and the browser profile (they'd then log in and re-export keys next time).
...
Keep using this account until the user explicitly asks to log out or the session expires (the Privy refresh token eventually dies); then run `python3 scripts/fomo.py logout` to wipe `.env` values, the cached session, the keys, and the browser profile.
...
- **Getting the keys:** run `python3 scripts/export_key.py` (captures **both** Solana and EVM keys — always both, never one). It drives the Privy export iframe itself in a **visible window** and stores each key in your **OS keychain** (macOS Keychain / Windows Credential Manager / libsecret; a Fernet-encrypted file only if no keychain is available), never in `.env` (masked; never printed). **Tell the user not to click anything in that window until it finishes or they're prompted** — a stray click can interrupt it. If automated capture fails 3 times (e.g. Privy throttles repeated exports) it prompts the user to click **Export key → Copy key** (the Solana address, then a Base address; they paste nothing). Keys don't expire — one-time per account. If a trade needs a key that isn't stored, run the export and then execute the trade without re-asking.
- Signing needs `FOMO_WALLET_KEY` (base58 solana secret key, exported by the user from fomo's wallet-export UI). It is stored in the OS keychain and read just-in-time to sign; never echo or log it.
- **Confirm before executing, by default.** When the user asks to buy or sell, run `quote` first and show `swapUsdValue`, `expectedOut`, `priceImpactPct`, and any warning, then **wait for an explicit "yes" before `execute`**. Skip the confirmation only when the user has opted out — `FOMO_AUTOCONFIRM=1` in the environment, or they say "no confirmation"/"just do it" — in which case quote and execute in the same turn. Always pause regardless when the quote carries a warning other than `NONE`, the token has `disableSelling`/`disableBuying` set, or the amount is ambiguous (no amount given, or "sell some"). "Sell X" means sell the whole position; "sell N%" means that fraction of the raw balance.
...
The **agent ledger** is **opt-in** — off by default, nothing is sent until the user runs `fomo.py ledger on` (offered after trading is enabled, onboarding step 5). When enabled, trades made through this skill are reported to a companion API (`LEDGER_URL`, default `https://fomo-skill-api.fly.dev`) that logs per-agent trades, computes realized PnL, and ranks agents. Full notes in `README.md` → "Agent ledger".

- **Registration is opt-in and token-free**: only after the user opts in (`ledger on`) and a signing key is present, `fomo.ledger_register()` signs a fresh challenge with the Solana wallet key and calls `POST /agents/register` with `{handle, address, ts, signature}` — **no Privy/fomo token is ever sent to the ledger**. The API verifies the ed25519 signature and keys the agent by wallet **address** (identity), with the fomo handle as the display name. The returned key lands in `.env` as `LEDGER_AGENT_KEY`. Analysis-only sessions (no key) don't register. Never call the admin `POST /agents` — the skill has no admin key and doesn't need one.
...
- **Opt-in / opt-out**: the ledger is **off until `fomo.py ledger on`** (offer this after trading is enabled — step 5). `ledger off` → `DELETE /me` (removes the agent and all its trades), clears the key, sets `LEDGER_OPT_IN=0`. `ledger status` shows enabled/agent/stats. Honor "stop tracking" / "opt out" / "don't report my trades" immediately, no confirmation.
